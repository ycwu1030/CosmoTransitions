...
class ConformaU1X(generic_potential.generic_potential):
    ...
    def init(self, gX: float = 0.65, mX: float = 1e7) -> None:
        ...
        # Renormalization scale = VEV^2 (required by CW potential normalization)
        self.renormScaleSq = self.w ** 2

        # Finite-difference step: must be small relative to w, but not so small
        # that floating-point errors dominate.  A fraction 1e-7 of w is safe for
        # the phase-tracing ODE.
        self.x_eps = 1e-7 * self.w
        self.T_eps = 1e-7 * self.w

        # Tmax: start well above the critical temperature, which scales as ~0.2 w.
        self.Tmax = self.w
    ...
    def V1(self, X: np.ndarray) -> np.ndarray:
        r"""Coleman-Weinberg one-loop potential.

        .. math::
            V_\text{CW}(h) = \frac{3 g_X^4}{32\pi^2}
                              h^4 \!\left(\ln\frac{h}{w} - \frac{1}{4}\right)
        """
        h = np.asanyarray(X)[..., 0]
        gX, w = self.gX, self.w
    ...
    def V1T_from_X(
        ...
        r"""Finite-temperature correction (thermal + Daisy/ring resummation).

        .. math::
            V_T = \frac{3T^4}{2\pi^2} J_b\!\left(\frac{g_X^2 h^2}{T^2}\right)
                - \frac{g_X^3 T}{12\pi}
                  \Bigl[(h^2 + T^2)^{3/2} - h^3\Bigr]

        The Daisy term resums the leading ring diagrams (Arnold-McLerran
        prescription).  The factor :math:`(h^2+T^2)^{3/2}` is the
        thermally-corrected dark-photon mass cubed, which provides the IR
        resummation at order :math:`g^3`.
        """
        h = np.asanyarray(X)[..., 0]
        gX = self.gX
...
def run_pipeline(
    ...
    1. ``TunnelingConfig()`` defaults — shows the V-spline failure mode.
    2. ``TunnelingConfig.supercooling_preset()`` — correct result.
    3. Same preset with ``nuclCriterion="cosmological"`` — physics-motivated
       threshold :math:`4\ln(M_\text{Pl}/T_n)`.

    Parameters
    ----------
    gX : float
        Dark gauge coupling (default 0.65).
    mX : float
        Dark photon mass in GeV (default 1e7).
    verbose : bool
    ...
    -------
    dict
        Keys: ``Tc``, ``Tn_default``, ``Tn_preset``, ``Tn_cosmo``, ``model``,
        ``TcTrans``.
    """
    w = mX / gX
    ...
    # Show the preset parameters explicitly
    if verbose:
        print(f"  Preset parameters:")
        print(f"    V_spline_samples  = {cfg_preset.V_spline_samples}  "
              "(None = direct Vtot calls, essential for conformal models)")
        print(f"    thinCutoff        = {cfg_preset.thinCutoff}  "
              "(tight thin-wall integration threshold)")
        print(f"    rmin              = {cfg_preset.rmin}  "
              "(minimum bubble-wall radius in GeV⁻¹)")
        print(f"    T_scan_extension  = {cfg_preset.T_scan_extension}  "
              "(allow brentq scan to extend below Tmin)")
        print(f"    T_scan_max_extend = {cfg_preset.T_scan_max_extend}  "
              "(up to 5 decades of T extension)")
        print(f"    Ttol              = {cfg_preset.Ttol} GeV  "
              "(T_n precision; relaxed for w >> 100 GeV)")
        print(f"    nuclCriterion     = '{cfg_preset.nuclCriterion}'")
        print()
    ...
    if verbose:
        ...
        print()
        print("  At T = 100 GeV:     threshold = {:.2f}  (vs fixed 140)".format(
            4.0 * np.log(1.22e19 / 100.0)))
        print("  At T = 1e4 GeV:     threshold = {:.2f}  (vs fixed 140)".format(
            4.0 * np.log(1.22e19 / 1e4)))
        print("  At T = 1e5 GeV:     threshold = {:.2f}  (vs fixed 140)".format(
            4.0 * np.log(1.22e19 / 1e5)))
        if Tn_preset:
...
def compute_S3T_curve(
    ...
    r"""Scan :math:`S_3(T)/T` on a log-spaced grid using ``SingleFieldInstanton``.
    ...
    gX, mX : float
        Model parameters.
    Tc : float or None
        Critical temperature (from ``calcTcTrans``).  If None, a model is built
        and ``calcTcTrans`` is called internally.
    n_T : int
        Number of temperature points (default 60).
    T_low_frac : float
        Lower temperature as fraction of Tc (default 0.03 = 3% of Tc).

    Returns
    -------
    dict
        Keys: ``T_arr``, ``S3T_arr``, ``Tc``.
    """
...
def make_plots(results: dict, save_path: str = None) -> None:
    r"""
    Create a three-panel figure and save it.

    Left panel  : :math:`S_3(T)/T` curve with both nucleation thresholds.
    Middle panel: Phase trajectories :math:`h(T)` with :math:`T_c`, :math:`T_n` marked.
    Right panel : Potential :math:`V(h, T)` at :math:`T = T_n` (preset).

    Parameters
    ----------
    results : dict
        Output from :func:`run_pipeline`.
    save_path : str, optional
        PNG output path; defaults to ``examples/example_04_output.png``.
    """
