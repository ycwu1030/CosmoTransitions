...
def run_pipeline(verbose: bool = True):
    """Run getPhases → calcTcTrans → findAllTransitions on testModel1."""
    m = model1()
...
def compare_deform_conv(verbose: bool = True):
    """Compare deform_fRatioConv=0.02 (default) vs 0.005 (tight) on T_n and S3/T."""
    if verbose:
        print("=" * 65)
...
def make_plots(m, TcTrans, TnTrans, save_path=None):
    """Three-panel figure: phase trajectories, 2D potential + path, and bounce profile."""
